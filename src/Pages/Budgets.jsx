...
function Budgets() {
    ...
    }, [selectedPeriod]); // Re-fetch when the month changes
    ...
    const handleSubmit = async (e) => {
        ...
        setError('');
        try {
            // Ensure the form uses the currently selected dashboard period
    ...
    // Helper to calculate progress bar width and color based on Assignment requirements
    const getProgressDetails = (spent, total, isExceeded) => {
        const percentage = total > 0 ? (spent / total) * 100 : 0;
        const width = Math.min(percentage, 100) + '%';
        
        let colorClass = 'bg-emerald-500'; // Safe
        if (isExceeded) colorClass = 'bg-rose-500'; // Exceeded limit alert
        else if (percentage >= 80) colorClass = 'bg-amber-400'; // Warning threshold
                ...
                <main className="flex-1 p-8 md:p-10">
                    <header className="mb-8 flex justify-between items-end">
                        ...
                        <div className="flex gap-4 items-center">
                            {/* Month Selector for Dynamic Tracking */}
                            <input 
                                type="month" 
                                    ...
                                    <div key={b.budgetId} className={`bg-white p-6 rounded-xl border-2 shadow-sm transition-all ${b.isExceeded ? 'border-rose-300 bg-rose-50' : 'border-slate-100 hover:border-indigo-200'}`}>
                                        <div className="flex justify-between items-start mb-4">
                                            <div>
                                                ...
                                                {/* Visual Alert Badge Requirement */}
                                                {b.isExceeded && <span className="text-xs font-bold text-rose-600 bg-rose-100 px-2 py-1 rounded-full uppercase tracking-wider mt-1 inline-block">Over Budget</span>}
                                            </div>
                                        ...
                                        <div className="w-full bg-slate-200 rounded-full h-3 mb-2 overflow-hidden">
                                            <div className={`h-3 rounded-full transition-all duration-500 ${colorClass}`} style={{ width: width }}></div>
                                        </div>
